...
def process_data(data_path,mask_path):
    ...
    data = data[drop_data:,:,:]
    data = np.reshape(data,[-1,batch_size,data.shape[1],data.shape[2]])
    #data = np.array(data)
    
    
    mask = np.load(mask_path) 
    mask = np.repeat(np.expand_dims(mask,axis=0),batch_size,axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument( '--config', type=str, default='./config/config_SSSDSA_diff.json',
